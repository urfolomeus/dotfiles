...
class SvgPreviwerContentProvider {
    ...
    createHtml(doc) {
        let bg = vscode.workspace.getConfiguration('svg.preview').get('background') || 'transparent';
        let bgCustom = vscode.workspace.getConfiguration('svg.preview').get('backgroundCustom') || '#eee';
        let svg = doc.getText();
        const html = [];
