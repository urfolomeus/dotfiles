...
function findFunctionOrClassByClassName(entryDocument, position, funcOrClass, clasName) {
    ...
    if (-1 !== lineIndex) {
        ...
        let line = beforeLines.find(line => new RegExp("^class\\s+.*" + clasName + SYMBOL_END).test(line.trim()));
        if (!line) {
            return Promise.reject('');
        }
        let filePath = getParentControllerFilePathByDocument(entryDocument, line), fileAbsPath = path.join(vscode.workspace.rootPath, filePath);
...
//# sourceMappingURL=rails_definition.js.map